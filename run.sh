...
check_code_deploy_deployment_group_name(){
...
check_code_deploy_skip(){
  if [ "$WERCKER_AWS_CODE_DEPLOY_SKIP" != "true" ] && [ "$WERCKER_AWS_CODE_DEPLOY_SKIP" != "false" ] ; then
    error "Please set the 'skip' variable to either: true | false"
    exit 1
  fi
}
